...
#include "crc16.h"
#define FLASH_PAGE_SIZE     ((uint16_t)NRF_FICR->CODEPAGESIZE)  /**< Size of one flash page. */
...
#define DFU_TIMEOUT_INTERVAL      APP_TIMER_TICKS(60000)                                            /**< DFU timeout interval in units of timer ticks. */   // JTN   
...
uint32_t dfu_init(void)
{
    ...
    if ((bootloader_settings.bank_1 != BANK_ERASED) || (*p_bank_start_address != EMPTY_FLASH_MASK))
    {
        ...
        {
            nrf_nvmc_page_erase((page + page_count)*FLASH_PAGE_SIZE);
        }
...
uint32_t dfu_data_pkt_handle(dfu_update_packet_t * p_packet)
{
    ...
    switch (m_dfu_state)
    {
        ...
        case DFU_STATE_RX_DATA_PKT:
            ...
            nrf_nvmc_write_words((uint32_t)mp_app_write_address, p_data, p_packet->packet_length);
...
uint32_t dfu_image_activate()
{
    ...
    {    
        case DFU_STATE_WAIT_4_ACTIVATE:
            ...
            for (page_count = 0; page_count < num_of_pages; page_count++)
            {
                nrf_nvmc_page_erase((page_start + page_count)*FLASH_PAGE_SIZE);
            }
        
            // Copying bytes from BANK 1 (SWAP) to BANK 0.
            num_of_words = m_image_size / sizeof(uint32_t);
            nrf_nvmc_write_words((uint32_t)p_bank_0_start_address, p_bank_1_start_address, num_of_words);
